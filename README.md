...
- **玩家单独开关**：每个玩家可单独关闭女仆的自言自语；房主关闭总开关后，玩家设置项自动置灰
...
提示词不在界面改，直接编辑 `config/maid_self_talk-common.toml` 的 `[prompt]` 段。
...
- **Per-player switch**: Each player can turn off a maid's self-talk individually; once the host disables the master switch, the player's option greys out automatically
...
- **Visible in chat**: Self-talk is visible to nearby players
...
Prompts are not editable in the UI — edit the `[prompt]` section of `config/maid_self_talk-common.toml` directly.
