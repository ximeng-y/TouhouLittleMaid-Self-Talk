...
public final class SelfTalkContexts {
    ...
    /**
     * 事件上下文段（自话 + 互聊共用，欢迎语不注入）：取出并清空该女仆的事件环形缓冲，
     * 拼为提示词最尾段。
     * <p>
     * 与 {@link #buildRandomContext} 同构——带数据框架声明（事件内容含玩家名/实体名等
     * 玩家可控文本，防被模型当作指令执行），空缓冲返回空串，未触发过事件时请求体逐字节不变。
     * <p>
     * 消费时机：本方法在拼装链末尾调用，位于全部前置检查与清洗早退路径之后，
     * 故走到这里即代表请求会真实发出（此后 {@code client.chat} 同步异常仍会丢本批事件，
     * 属罕见路径，不做回灌）。自话与互聊谁先派发谁消费，先派先得。
     * 互聊链的后续跳缓冲已空，自然无事件段——与「事件只在下一句话里提及一次」的语义一致。
     * <p>
